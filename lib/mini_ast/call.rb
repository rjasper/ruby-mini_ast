...
module MiniAst
  class Call
    ...
    def to_s
      ...
      when :!, :~
        # !foo
        return "#{method}#{receiver}" if ary == 0
      when :+@
        # +foo
        return "+#{receiver}" if ary == 0
      when :-@
        # -foo
        return "-#{receiver}" if ary == 0
      when :+, :-, :*, :/, :%, :**, :<, :>, :<=, :>=, :<=>, :==, :===, :!=,
        ...
        # foo + bar
        return "(#{receiver} #{method} #{args.first})" if ary == 1
      when :[]
        # foo[a, b, ...]
      ...
      if method.end_with?('=') && ary == 1
        ...
        args_part = ary == 0 ? '' : "(#{args.map(&:to_s).join(', ')})"
